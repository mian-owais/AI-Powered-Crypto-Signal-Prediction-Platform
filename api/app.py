...
def train():
    ...
    try:
        crypto = request.json.get(
            "crypto", "bitcoin") if request.json else "bitcoin"
        force = request.json.get(
            "force_retrain", False) if request.json else False

        model_data, status = get_or_train_model(
            crypto=crypto, force_retrain=force)
